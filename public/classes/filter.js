...
export function setupSearch() {
    ...
    searchButton.addEventListener("click", () => {
        ...
        const filteredTasks = tasks.filter((task) => task.category.toLowerCase().includes(query) ||
...
export function setupApplyFilter() {
    ...
    applyFilterButton.addEventListener("click", () => {
        ...
        const selectedCategories = Array.from(document.querySelectorAll(".selected-categories .category-tag")).map((tag) => tag.textContent?.replace(" ×", "") || "");
        // ✅ Get date range inputs
        const fromDateInput = document.querySelector(".date.from");
        const toDateInput = document.querySelector(".date.to");
        ...
        const minAmountInput = document.querySelector(".amount.min");
        const maxAmountInput = document.querySelector(".amount.max");
        ...
        const filteredTasks = tasks.filter((task) => {
            ...
            return ((selectedCategories.length === 0 || selectedCategories.includes(task.category)) &&
